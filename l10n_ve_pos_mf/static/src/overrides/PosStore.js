 ...
 *
 * Migración v17 -> Odoo 19: nombres de métodos/getters renombrados (ver tabla
 * canónica en l10n_ve_pos/openspec/.../apply-progress.md, sección "Odoo 19 API
 * changes reference"). Puntos clave:
 *   - order.get_total_with_tax() -> order.totalDue
 *   - order.get_partner()        -> order.getPartner()
 *   - order.is_paid_with_cash()  -> order.isPaidWithCash()
 *   - order.uid                 -> order.uuid
 *   - order.orderlines/paymentlines -> order.lines / order.payment_ids (colecciones directas)
 *   - line.get_discount()/get_product() -> line.getDiscount()/getProduct()
 *   - line.product / line.quantity -> line.product_id / line.qty
 *   - line.payment_method        -> line.payment_method_id
 *   - push_single_order(order, opts) -> pushSingleOrder(order) (firma cambiada; opts ya no existe)
 *   - Refund tracking: reemplazado el dict global v17 `toRefundLines` por la
 *     relación directa `line.refunded_orderline_id` (Many2one) en Odoo 19.
 */
patch(PosStore.prototype, {
  ...
  /**
   * Busca la orden/línea original que esta orden está reembolsando.
   *
   * Odoo 19: la relación `line.refunded_orderline_id` (Many2one) reemplaza
   * el dict global v17 `toRefundLines`. Si la orden original está cargada
   * en memoria (related_models), obtenemos sus datos fiscales directamente
   * sin RPC. Si no lo está (orden de una sesión anterior), recurrimos al
   * historial local o al backend (get_order_by_uid), igual que en v17.
   *
   * @param {Object} order
   * @returns {{hasRefundLines: boolean, originalOrderUuid: string|null, originalOrder: Object|null}}
   */
  _getRefundInfo(order) {
    const refundLines = (order.lines || []).filter((line) => line.refunded_orderline_id);
    if (!refundLines.length) {
      return { hasRefundLines: false, originalOrderUuid: null, originalOrder: null };
    }
    const originalLine = refundLines[0].refunded_orderline_id;
    const originalOrder = originalLine?.order_id || null;
    return {
      hasRefundLines: true,
      originalOrderUuid: originalOrder?.uuid || null,
      originalOrder,
    };
  },
  ...
  async get_data_invoice(order) {
    ...
    const hasRefundLines = refundInfo.hasRefundLines;
    ...
    if (hasRefundLines && invoice['type'] == 'out_refund') {
      // 1. Intentar leer directamente de la orden original ya cargada en memoria
      if (refundInfo.originalOrder && refundInfo.originalOrder.mf_invoice_number) {
        affectedOrderData = {
          mf_invoice_number: refundInfo.originalOrder.mf_invoice_number,
          fiscal_machine: refundInfo.originalOrder.fiscal_machine,
          date_order: refundInfo.originalOrder.date_order,
          payment_lines: Array.from(refundInfo.originalOrder.payment_ids || []).map((p) => ({
      ...
      // 2. Historial local (pedidos impresos offline en esta sesión de navegador)
      if (!affectedOrderData) {
        const localOrder = LocalOrderHistory.getByUid(refundInfo.originalOrderUuid);
        if (localOrder) {
          affectedOrderData = localOrder;
        }
      }

      // 3. Backend (orden de una sesión anterior no cargada en memoria)
      if (!affectedOrderData) {
        try {
          const response = await this.orm.call("pos.order", "get_order_by_uid", [[], refundInfo.originalOrderUuid])
    ...
    if (order.lines.length > 0) {
      ...
      invoice['invoice_lines'] = order.lines.map((el) => {
        const customerNote = el.customer_note || (typeof el.getCustomerNote === "function" ? el.getCustomerNote() : "");
        if (customerNote) {
          let split = customerNote.split("\n")
          for (let i = 0; i < split.length; i++) {
            invoice["info"].push(`${split[i]}`)
          }
        }

        let amount = vef_base ? (el.price ?? el.price_unit) : el.get_foreign_unit_price?.()
        const taxes = el.tax_ids || []
        ...
        return {
          ...
          name: this.normalizeProductName(el.getProduct().display_name),
          code: el.getProduct().default_code,
          tax: fiscalCode,
        }
      })

      invoice['payment_lines'] = Array.from(order.payment_ids || [])
        .map((el) => {
          let amount = vef_base ? el.amount : el.get_foreign_amount?.()
          return {
            payment_method: el.payment_method_id?.code_fiscal_printer || false,
            ...
            _debug_apply_igtf: Boolean(el.payment_method_id?.apply_igtf),
          }
        })
        .filter((line) => {
      ...
      // documento fue pagado en su totalidad).
      {
        ...
        const totalWithTax = order.totalDue ?? (typeof order.get_total_with_tax === "function" ? order.get_total_with_tax() : 0);
        console.log("PosStore:: [IGTF-DEBUG] get_data_invoice para orden", order.uuid);
        console.log("PosStore:: [IGTF-DEBUG] order.totalDue:", totalWithTax);
        ...
        console.log("PosStore:: [IGTF-DEBUG] Diferencia (total_with_tax - suma_pagos):", Number(totalWithTax || 0) - sumPayments);
      }
      ...
      if (
        ...
        const refundTotal = Math.abs(
          roundAmount(
            vef_base
              ? (order.totalDue ?? order.get_total_with_tax?.())
  ...
  _isGlobalDiscountProductLine(line) {
    ...
    if (!discountProductId || !line?.getProduct) {
      return false;
    }
    return Number(line.getProduct()?.id || 0) === Number(discountProductId);
  },
  ...
  _collectGlobalDiscountMeta(order) {
    ...
    for (const line of allLines) {
      const quantity = Math.abs(Number(line.qty ?? 0));
      if (!quantity) {
        continue;
      }
  ...
  _applyGlobalDiscountBeforeValidation(order, { force = false } = {}) {
    const hasPendingDiscountLines = [...(order.lines || [])].some(
      (line) => this._isGlobalDiscountProductLine(line) && Number(line.price_unit ?? 0) < 0
    );
    ...
    for (const line of positiveLines) {
      const currentDiscount = Number(
        line._mf_base_discount !== undefined
          ...
          : (line.getDiscount?.() ?? line.discount ?? 0)
      );
      ...
      if (typeof line.setDiscount === "function") {
        line.setDiscount(composedDiscount);
      } else {
  ...
  /**
   * Override de pushSingleOrder con soporte offline-first.
   *
   * Odoo 19: la firma cambió de `push_single_order(order, opts)` a
   * `pushSingleOrder(order)` (sin `opts`; internamente delega en
   * `syncAllOrders(order)` vía mutex).
   *
   * Flujo:
   * 1. Aplicar descuento global pendiente (por si no se recomputó reactivamente)
   * 2. Validación contable (dry-run) - tolerante a fallos de red
   * 3. Impresión fiscal (offline) - SIEMPRE se ejecuta
   * 4. Sincronización con backend - con buffer offline si falla
   */
  async pushSingleOrder(order) {
    ...
    try {
      const order_payload = [{
        'data': order.serializeForORM ? order.serializeForORM() : order.export_as_JSON?.()
      }];
    ...
    try {
      return await super.pushSingleOrder(order);
    } catch (syncError) {
      ...
      const orderData = order.serializeForORM ? order.serializeForORM() : order.export_as_JSON?.();
      const fiscalData = {
