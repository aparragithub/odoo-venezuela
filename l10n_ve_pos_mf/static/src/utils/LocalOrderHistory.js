...
export class LocalOrderHistory {
    ...
    static add(order) {
        ...
        const paymentLines = Array.from(order.payment_ids || []).map((paymentLine) => ({
            amount: paymentLine.amount,
            payment_method_code: paymentLine.payment_method_id?.code_fiscal_printer || false,
            payment_method_name: paymentLine.payment_method_id?.name || "",
        }));
