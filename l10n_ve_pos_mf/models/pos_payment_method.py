...
class PosPaymentMethod(models.Model):
    ...
    def _load_pos_data_fields(self, config):
        """Odoo 19 loader contract: incluir ``code_fiscal_printer`` para que
        el driver Web Serial (TfhkaDriver) mapee cada línea de pago al
        código fiscal correspondiente en la impresora TFHKA.
        """
        res = super()._load_pos_data_fields(config)
        if "code_fiscal_printer" not in res:
            res.append("code_fiscal_printer")
        return res
