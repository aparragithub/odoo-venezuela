...
class PosOrderInherit(models.Model):
    ...
    def _load_pos_data_fields(self, config):
        """Odoo 19 loader contract (reemplaza los hooks Odoo 17
        ``_order_fields``/``_export_for_ui``, eliminados de core y de
        ``l10n_ve_pos`` en la migracion base — Slice B).
        """
        res = super()._load_pos_data_fields(config) or []
        for field_name in ("fiscal_machine", "mf_invoice_number", "mf_reportz"):
